...
if ($action == "viewposts")
{
	...
	$res = mysql_query("SELECT username, donor, warned FROM users WHERE id=$userid") or sqlerr(__FILE__, __LINE__);
	...
	if (mysql_num_rows($res) == 1)
	{
	  ...
	  $subject = "<a href=userdetails.php?id=$userid><b>$arr[username]</b></a>".
	  	($arr["donor"] == "yes" ? "<img src=pic/starbig.gif alt='Donor' style='margin-left: 4pt'>" : "") .
	  	($arr["warned"] == "yes" ? "<img src=pic/warnedbig.gif alt='Warned' style='margin-left: 4pt'>" : "");
	}
	else
	...
	$select_is = "f.id AS f_id, f.name, t.id AS t_id, t.subject, p.*";
...
//	    if (strlen($topicname) > 40)
//	    	$topicname = substr($topicname,0 ,37)."...";
