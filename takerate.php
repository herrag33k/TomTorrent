...
$ratedbefore_sql = 'SELECT COUNT(*) FROM ratings WHERE torrent = '.$id.' AND user = '.$CURUSER['id'];
$ratedbefore = mysql_result(mysql_query($ratedbefore_sql),0);

if($ratedbefore < '1') {
	$res = mysql_query("INSERT INTO ratings (torrent, user, rating) VALUES ($id, " . $CURUSER["id"] . ", $rating)");
	if (!$res) {
		if (mysql_errno() == 1062)
